    ...
    <div class="table-box">
      ...
      <a-modal v-model="dialogVisible" title="新增" ok-text="确认" cancel-text="取消" width="500px" @ok="addTask">
        <a-form-model ref="form" layout="horizontal" :model="form" :rules="form_rule" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
          ...
          <a-form-model-item label="选择资源"  v-if="type == 2">
          ...
          <a-form-model-item v-for="(domain, index) in form.card_text" v-if="form.resourceName == '' && type == 5" :key="index + 'info3'" :label="'文本'" class="short-input btn-item" prop="card_text">
            <a-input v-model="domain.value"/>
      ...
      if(this.form.text[0].value === '') {
        if(this.type === 2) {
          this.form.text =this.form.text.map( item=> {
            return { value: valueas[0].data.join(',')}
          })
        }
      } else {
        console.log(this.form.text)
        let objvalue = {
          value: valueas[0].data.join(',')
        }
      ...
      api.postTiktokParseNameCardUser({ url: this.nameCardFrom.url }).then((res) => {
        if (res.code === 0) {
          ...
          if(this.form.name_card[0].value === '') {
            ...
            this.form.name_card = this.form.name_card.map( item=> {
              return {...item, value: JSON.stringify(datas)}
            })
            // this.form.name_card.push({ value: JSON.stringify(res.data) })
          }
          console.log(this.form.name_card)
